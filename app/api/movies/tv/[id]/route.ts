...
const getUrlVideo = async (idMovie: number): Promise<string | null> => {
  try {
    const response = await axios.get(
      `${process.env.BASE_URL_API}tv/${idMovie}/videos?api_key=${process.env.API_KEY}&language=en-US`
    );
...
const getSimilarMovies = async (id: number): Promise<any[]> => {
  try {
    const response = await axios.get(
      `${process.env.BASE_URL_API}tv/${id}/similar?&api_key=${process.env.API_KEY}&language=fr`
    );
    if (response.status === 200) {
